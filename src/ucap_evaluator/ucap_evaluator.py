...
class UCAPEvaluator:
    ...
    def __init__(self,config:dict):
        ...
        self.hour_filter = pd.read_csv(config['ucap_analysis']['hour_filter_path'])
        self.hour_filter.loc[:,'DATETIME'] = pd.to_datetime(self.hour_filter.loc[:,'DATETIME'])
    ...
    def calculate_equivalent_forced_outage_rate_during_demand_hours(self,hour_filter:list):
        '''
        Filters curtailment data for the input range of dates, then evaluates
        the forced outage rate for the month based on curtailment data given the
        nature-of-work codes included in the configuration file.

            parameters:
                hour_filter - a pandas dataframe with columns labeled 'DATETIME'
                    and 'INCLUDE' indicating selected hours, generally expected
                    to constitute a year of hours.
        ...
        # range:
        self.status_logger.log('\tFiltering curtailment reports.',criticality='INFORMATION')
        start_datetime = hour_filter.loc[:,'START DATETIME'].min().to_pydatetime()
        end_datetime = hour_filter.loc[:,'END DATETIME'].max().to_pydatetime()
        df.loc[:,'INCLUDE'] = df.apply(
            lambda r: datetime_range_overlap(
                        start_datetime,
                        end_datetime,
        ...
        df = df.loc[
            (df.loc[:,'CURTAILMENT START DATE TIME']<=hour_filter.loc[:,'END DATETIME'].max()) \
            * (df.loc[:,'CURTAILMENT END DATE TIME']>=hour_filter.loc[:,'START DATETIME'].min()),
            :
        ]
        df.loc[:,'APPLICABLE OUTAGE HOURS'] = df.apply(
